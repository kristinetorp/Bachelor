...
df_RRST_sub <- read_csv("RRST_DEMO.csv")
df_RRST_sub$gender <- as.factor(df_RRST_sub$gender)
df_RRST_sub$smoking <- as.factor(df_RRST_sub$smoking)
...
ggplot(df_RRST_sub, aes(x = age)) +
  geom_histogram(aes(y=..density..),binwidth=0.25)+
  stat_function(fun = dnorm,args = list(mean = mean(df_RRST_sub$age, na.rm = TRUE), sd = sd(df_RRST_sub$age, na.rm = TRUE)), colour = "darkgreen", size = 1)+
  labs(title = "Age distribution")

mean(df_RRST_sub$bmi, na.rm = TRUE)
sd(df_RRST_sub$bmi, na.rm = TRUE)

mean(df_RRST_sub$height, na.rm = TRUE)
sd(df_RRST_sub$height, na.rm = TRUE)
ggplot(df_RRST_sub, aes(x = height)) +
  geom_histogram(aes(y=..density..),binwidth=0.25)+
  stat_function(fun = dnorm,args = list(mean = mean(df_RRST_sub$height, na.rm = TRUE), sd = sd(df_RRST_sub$height, na.rm = TRUE)), colour = "darkgreen", size = 1)
...
count(df_RRST_sub$smoking == "5")
ggplot(df_RRST_sub, aes(x = smoking)) +
  geom_histogram(aes(y=..density..),binwidth=0.25)+
  stat_function(fun = dnorm,args = list(mean = mean(df_RRST_sub$smoking, na.rm = TRUE), sd = sd(df_RRST_sub$smoking, na.rm = TRUE)), colour = "darkgreen", size = 1)
